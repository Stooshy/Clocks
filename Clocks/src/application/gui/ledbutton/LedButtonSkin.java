...
public class LedButtonSkin extends SkinBase<LedButton> implements Skin<LedButton>
{
	...
	private void initGraphics()
	{
		...
		ledOnInnerShadow.setColor(getSkinnable().getLedColor().darker().darker());
	...
	private void registerListeners()
	{
		...
		((LedButton) getSkinnable()).ledColorProperty().addListener(new InvalidationListener()
		{
			@Override
			public void invalidated(Observable observable)
			{
	...
	private void resize()
	{
		...
		font = Font.font("Arial", FontWeight.BOLD, FontPosture.REGULAR, 40.0 / 144.0 * size);
		text.setFont(font);

	...
	public Buttons(Stage stage, ScreensController mainContainer)
	{
		...
		go.selectedProperty().addListener(new ChangeListener<Boolean>()
		{
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue)
			{
				...
				{
					Updateable counter = (Updateable) mainContainer.getScreen().screen;
					counter.toggleMode();
				}
			}
		});
		...
		counterMode.selectedProperty().addListener(new ChangeListener<Boolean>()
		{
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue)
			{
				...
				{
					counterMode.setSkinText(">");
					((SevenSegmentsDisplayStopWatch) mainContainer.getScreen().screen).setMode(CounterMode.UP);
				}
				else
				{
					counterMode.setSkinText("<");
					((SevenSegmentsDisplayStopWatch) mainContainer.getScreen().screen).setMode(CounterMode.DOWN);
				}
			}
		});
